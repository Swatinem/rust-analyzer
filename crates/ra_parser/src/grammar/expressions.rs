...
fn lhs(p: &mut Parser, r: Restrictions) -> Option<(CompletedMarker, BlockLike)> {
    ...
    let kind = match p.current() {
        ...
        _ => {
            ...
            return Some(postfix_expr(p, lhs, blocklike, !(r.prefer_stmt && blocklike.is_block()), r.forbid_structs));
        }
    };
    expr_bp(p, r, 255);
    ...
    fn postfix_dot_expr(
        ...
        for &(op, la) in &[(T![..=], 3), (T![..], 2)] {
            if p.at(op) {
                ...
                let has_trailing_expression = !(forbid_structs && next_token == T!['{']) && EXPR_FIRST.contains(next_token);
